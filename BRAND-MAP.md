...
| **The album pipeline** *(formerly "Album OS" — retired as an outward name)* | Internal name for the 8-stage intake -> canon-tag -> curate -> sequence -> master -> cover -> release -> monetize pipeline (`schemas/album.schema.json`, `tools/album-builder.mjs`, `tools/tag-arcanea-guardian.mjs`). | product *(internal tooling — never publicly named)* | Internal only (Frank + agents); not an external-facing surface | None — do not use "Album OS" in public-facing copy, per D1 | The pipeline that turns a pile of Suno exports into a released, sequenced album with matching visuals (`docs/engineering/2026-07-album-os.md` §1). |
